...
function QuotationUploadfile({ isOpen, closeModal }) {
  ...
  const [file, setFile] = useState(null);
  const [uploadProgress, setUploadProgress] = useState(0);
  const [QutationType] = useState("D-martqutation"); // Set default QutationType
  const [uploadInterval, setUploadInterval] = useState(null);
  const [isUploading, setIsUploading] = useState(false);
      ...
      {isOpen && (
        <div className="fixed inset-0 p-4 flex flex-wrap justify-center items-center w-full h-full z-[1000] before:fixed before:inset-0 before:w-full before:h-full before:bg-[rgba(0,0,0,0.5)] overflow-auto font-[sans-serif]">
          <div className="w-full max-w-lg bg-white shadow-lg rounded-lg p-6 relative">
            <div className="flex items-center pb-3 border-b border-gray-200">
              <div className="flex-1">
                <h3 className="text-gray-800 text-xl font-bold">Upload Quotation File</h3>
            ...
            <form onSubmit={handleSubmit} className="max-w-sm mx-auto">
              ...
              {file && (
                <div className="flex flex-col bg-gray-50 p-4 rounded-lg mt-4">
                  ...
                  <div className="relative pt-2">
                    ...
                    <div className="text-right text-xs text-gray-600 mt-1">{uploadProgress}%</div>
                  </div>
                </div>
              )}
              ...
              <div className="text-right space-x-2 mt-4">
                ...
                <button
                  ...
                  className={`${
                    isUploading ? "bg-gray-300 cursor-not-allowed" : "bg-blue-600 hover:bg-blue-700"
                  } text-white px-6 py-2 rounded-lg`}
                >
                  {isUploading ? "Uploading..." : "Upload File"}
                </button>
